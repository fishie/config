...
function script:gitBranches($filter, $includeHEAD = $false, $prefix = '', $includeRemote = $true) {
    ...
    if ($includeHEAD) {
        $branches += @('HEAD','FETCH_HEAD','ORIG_HEAD','MERGE_HEAD')
    }

    if ($includeRemote) {
        $branches += @(git branch --no-color -r | ForEach-Object { if ($_ -match "^  (?<ref>\S+)(?: -> .+)?") { $matches['ref'] } })
    }
...
function GitTabExpansionInternal($lastBlock, $GitStatus = $null) {
    ...
    switch -regex ($lastBlock -replace "^$(Get-AliasPattern git) ","") {
        ...
        "^branch.* (?<branch>\S*)$" {
            gitBranches $matches['branch'] $false '' $false
        }
        ...
        # Handles git checkout|switch
        "^(?:checkout|switch).* $" {
            & {
                gitBranches $matches['ref'] $false '' $false
